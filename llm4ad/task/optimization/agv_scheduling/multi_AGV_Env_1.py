...
class multiAGV_Env:
    ...
    def return_near_finished_part_pos_in_B(self):
        # Look for parts that have finished processing
        for i in range(self.B_info.shape[0]):
            # Check if there's a part and it has finished processing
            if self.B_info[i,0] >= 1 and self.AGV_timer[0] >= self.B_info[i,2]:
                return i
            # If no tray has a processed part but there's a tray with just a tray (no part)
            elif self.B_info[i,0] == 0 and self.B_info[i,1] > 0:
                return i
                
        # Fallback - return the position with the shortest remaining processing time
        _list = self.B_info[:,2]
        shortest_time = float('inf')
        index = 0
        for i in range(len(_list)):
            if _list[i] > 0 and _list[i] < shortest_time:
                shortest_time = _list[i]
                index = i
        return index
    ...
    def check_action1_valide(self):
        # Check if any parts in B have finished processing
        for i in range(self.B_info.shape[0]):
            # Debug: Print B position state
            print(f"DEBUG B[{i}]: part={self.B_info[i,0]}, tray={self.B_info[i,1]}, time={self.B_info[i,2]}, current={self.AGV_timer[0]}")
            
            # A part has finished processing if there's a part (B_info[i,0] >= 1) 
            # and its processing time has elapsed (current AGV timer >= B_info[i,2])
            if self.B_info[i,0] >= 1 and self.AGV_timer[0] >= self.B_info[i,2]:
                print(f"DEBUG: Action 1 valid - part finished processing at B[{i}]")
                return True
            # Also check for trays at B that don't have a part (part already processed)
            elif self.B_info[i,0] == 0 and self.B_info[i,1] > 0:
                print(f"DEBUG: Action 1 valid - empty tray at B[{i}]")
                return True
        print("DEBUG: Action 1 invalid - no recyclable trays found at B")
        return False
