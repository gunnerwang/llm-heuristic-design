...
class HttpsApi(LLM):
    ...
    def draw_sample(self, prompt: str | Any, *args, **kwargs) -> str:
        ...
        while True:
            try:
                # Parse host to extract domain and path
                parsed_url = urllib.parse.urlparse('https://' + self._host)
                domain = parsed_url.netloc
                path = parsed_url.path
                
                # Establish connection
                conn = http.client.HTTPSConnection(domain, timeout=self._timeout)
                
                # Handle different API formats based on host
                if 'dashscope.aliyuncs.com' in domain:
                    ...
                    payload = json.dumps({
                        ...
                        'parameters': {
                            ...
                            'top_p': self._kwargs.get('top_p', 0.8),
                    ...
                    # Default OpenAI-like API format
                    payload = json.dumps({
                        'max_tokens': self._kwargs.get('max_tokens', 4096),
                    ...
                    conn.request('POST', path or '/v1/chat/completions', payload, headers)
                    res = conn.getresponse()
                    data = res.read().decode('utf-8')
                    data = json.loads(data)
