...
def run_comparative_experiments():
    ...
    # 打印对比表格
    print(f"\n{'实验名称':<20} {'平均最佳分数':<12} {'分数标准差':<10} {'平均成本($)':<12} {'平均Token':<10} {'评估次数':<8} {'成功率':<8} {'改进(%)':<8}")
    print("-" * 100)
    for row in comparison_table:
        print(f"{row['experiment']:<20} {row['avg_best_score']:<12.3f} {row['score_std']:<10.3f} {row['avg_cost']:<12.6f} {row['avg_tokens']:<10.0f} {row['avg_evaluations']:<8.0f} {row['success_rate']:<8.3f} {row['improvement']:<8.1f}")
    ...
    print(f"  最高效率: {best_by_efficiency['experiment']} (成本效率: {best_by_efficiency['avg_cost']/max(best_by_efficiency['avg_best_score'], 1e-8):.6f})")
